...
-- large
	if ENABLE_SHARK_LARGE then
		mobs:register_mob("mobs_sharks:shark_lg", {
			...
			stepheight = 1,
			attack_animals = true,
			specific_attack = {"player", "mobs_mc:squid", "mobs_fish:clownfish", "mobs_fish:tropical"},
			drops = {
		...
		mobs:spawn_specific("mobs_sharks:shark_lg", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -200, -30)
...
-- medium
	if ENABLE_SHARK_MEDIUM then
		mobs:register_mob("mobs_sharks:shark_md", {
			...
			stepheight = 1,
			attack_animals = true,
			specific_attack = {"mobs_mc:squid", "mobs_fish:clownfish", "mobs_fish:tropical"},
			drops = {
				{name = "mobs:meat_raw", chance = 1, min = 1, max = 3},
			},
			fly = true,
		...
		--name, nodes, neighbours, minlight, maxlight, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_sharks:shark_md", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -200, -20)
...
-- small
	if ENABLE_SHARK_SMALL then
		...
		--name, nodes, neighbours, minlight, maxlight, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_sharks:shark_sm", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -300, -10)
		mobs:register_egg("mobs_sharks:shark_sm", "Shark (small)", l_egg_texture, 0)
	end

end

print ("[MOD]: mob_shark loaded")